...
const font2 = await createGoogleFont("Karla");
...
const charsmono = Freetype.LoadGlyphsFromCharmap(
    0,
    9999,
...
console.assert(
    setf.family_name === "Karla",
    "🔴 Font set returned value",
    setf
);
console.assert(font[0].family_name === "Karla", "🔴 Font should load", font);
console.assert(
    font2[0].family_name === font[0].family_name,
    "🔴 Font did not reload and replace",
    font2
);
...
console.assert(
    charsmono.size > 1,
    "🔴 Monochrome glyphs not loaded",
    chars.size
);
console.assert(
    chard?.bitmap.imagedata != null,
    "🔴 Antialiased data does not exist"
);
console.assert(
    chard.bitmap.pixel_mode === 2,
    "🔴 Gray mode is not enabled",
    chard.bitmap.pixel_mode
);
console.assert(
    monod?.bitmap.imagedata != null,
    "🔴 Monochrome data does not exist"
);
console.assert(
    monod.bitmap.pixel_mode === 1,
    "🔴 Monochrome data is not monochrome",
    monod.bitmap.pixel_mode
);
...
console.assert(null === Freetype.SetFont("Karla", "Regular"), " 🔴 Failure");
Freetype.Cleanup();
